...
def command_reconcile(args: argparse.Namespace, settings: Settings) -> int:
    ...
    unknown = forced - configured
    if unknown:
        raise ConfigError(f"unknown forced view(s): {', '.join(sorted(unknown))}")
    if args.allow_stale_build and not (forced or args.force_all):
        raise ConfigError("--allow-stale-build requires --force or --force-all")
    ...
    for view in views:
        ...
        try:
            revision = reconcile_view(
                view,
                settings,
                cache_root,
                public_views,
                force=force,
                allow_stale_build=args.allow_stale_build and force,
            )
            ...
            continue
        published.append((view, revision))
